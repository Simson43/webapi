...
namespace ConsoleApp
{
    class Program
    {
        ...
        private readonly ITurnsRepository turnRepo;
        ...
        private Program(string[] args)
        {
            ...
            var db = mongoClient.GetDatabase("web-game-tests");
            userRepo = new MongoUserRepository(db);
            gameRepo = new MongoGameRepository(db);
            turnRepo = new MongoTurnsRepository(db);
        }
        ...
        {
            if (humanUser.CurrentGameId == null)
                return null;
        ...
        {
            // Вместо этого кода можно написать специализированный метод в userRepo, 
            //      который сделает все эти обновления за одну операцию UpdateMany.
            // Вместо 4 запросов к БД будет 1, но усложнится репозиторий. В данном случае, 
            //      это редкая операция, поэтому нет смысла оптимизировать.
        ...
        private void ShowScore(GameEntity game)
        {
            ...
            // TODO: Показать информацию про 5 последних туров: кто как ходил и кто в итоге выиграл. 
            // Прочитать эту информацию из ITurnsRepository
            var lastTurns = turnRepo.GetFiveLastTurnEntity(game);
            foreach (var turn in lastTurns)
                Console.WriteLine($"Turn {turn.Index}: " +
                    $"{(turn.WinnerId == players[0].UserId ? "(WIN) " : "")}{players[0].Name} " +
                    $"{turn.GetDecisionByUserId(players[0].UserId)} : " +
                    $"{(turn.WinnerId == players[1].UserId ? "(WIN) " : "")}{players[1].Name} " +
                    $"{turn.GetDecisionByUserId(players[1].UserId)}");
